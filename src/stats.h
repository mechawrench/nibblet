...
inline void statsLoad() {
  ...
  _prefs.end();
  // Legacy approval-based level with no token count: seed tokens so the
  // new derivation agrees. Both systems were committed same-day so this
  // only matters on a dev device, but it's cheap and correct.
...
struct Settings {
  ...
  uint8_t clockMode;  // 0=time of day, 1=idle timer (since last claude code)
};
...
inline void settingsLoad() {
  ...
  _settings.clockMode = _prefs.getUChar("s_cmod", 0);
  if (_settings.clockMode > 1) _settings.clockMode = 0;
  _prefs.end();
}
...
inline void settingsSave() {
  ...
  _prefs.putUChar("s_cmod", _settings.clockMode);
  _prefs.end();
}
