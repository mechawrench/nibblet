...
// BLE replaces BT Classic SPP. See ble_bridge.cpp for the NUS GATT server.
// Legacy Classic pairing on modern macOS is unreliable — link keys don't
// persist and every reconnect fails with SDP timeout. BLE pairs cleanly,
// has proper bond persistence, and Web Bluetooth from the Electron
// renderer works without any CU-device or Python-serial shim.
// Advertise as "Nibblet-XXXX" (last two BT MAC bytes) so multiple sticks
// in one room are distinguishable in the desktop picker. Name persists in
// btName for the BLUETOOTH info page.
static char btName[16] = "Nibblet";
static void startBt() {
  uint8_t mac[6] = {0};
  esp_read_mac(mac, ESP_MAC_BT);
  snprintf(btName, sizeof(btName), "Nibblet-%02X%02X", mac[4], mac[5]);
  bleInit(btName);
}
...
static void applyReset(uint8_t idx) {
  ...
  if (idx == 0) {
    ...
    // factory reset: NVS namespace wipe + filesystem format. Clears
    // stats, owner, petname, species, settings, GIF characters. BLE
    // bonds live in a separate NVS partition the bridge owns — untouched.
...
void setup() {
...

  {
    ...
    if (ownerName()[0]) {
      ...
      spr.setTextSize(1);
      spr.setTextColor(p.textDim, p.bg);
      spr.drawString("a nibblet appears", W/2, H/2 + 12);
    }
  ...
  Serial.printf("nibblet: %s\n", buddyMode ? "ASCII buddy mode" : "GIF character loaded");
}
